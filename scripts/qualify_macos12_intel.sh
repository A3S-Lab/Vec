...
A3S_VEC_BENCH_SCALE=smoke RAYON_NUM_THREADS=1 \
    cargo +stable bench --locked --bench lifecycle_matrix --quiet \
    > "${performance_dir}/lifecycle-matrix.csv"
...
awk -F, -f .github/check_lifecycle_matrix.awk "${performance_dir}/lifecycle-matrix.csv"
